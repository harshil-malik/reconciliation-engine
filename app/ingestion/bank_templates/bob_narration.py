...
# Formats seen on a Bank of Baroda current-account statement.
...
#   CHEQUE DEPOSITED               (the reference lives in the separate Cheque No
#                                    column here, not in the narration)
...
_ATM = re.compile(r"^ATM\s+CASH\s+WDL\b", re.I)
...
def parse_narration(description: str) -> ParsedNarration:
    """Pull the transaction identifier and counterparty out of a Bank of Baroda
    narration.
    ...
    if _CHEQUE.match(text):
        # The reference comes from the statement's Cheque No column, not this cell.
        return ParsedNarration("cheque", None, None)
